...
def residual_block(previous):
    ...
    conv1 = Conv2D(256, kernel_size=3, strides=1, activation='relu', kernel_regularizer=l2(REG_PARAM), padding="same", data_format="channels_first")(previous)
    norm1 = BatchNormalization(axis=1)(conv1)  # Batch normalisation
    rnln1 = ReLU()(norm1)  # A rectifier non-linearity
    # A convolution of 256 filters of kernel size 3 × 3 with stride 1
    conv2 = Conv2D(256, kernel_size=3, strides=1, activation='relu', kernel_regularizer=l2(REG_PARAM), padding="same", data_format="channels_first")(rnln1)
    norm2 = BatchNormalization(axis=1)(conv2)  # Batch normalisation
...
class Network:
    def __init__(self):
        self.input = Input(shape=(M*T + L, N, N))

        # A convolution of 256 filters of kernel size 3 × 3 with stride 1
        conv = Conv2D(256, kernel_size=3, strides=1, activation='relu', kernel_regularizer=l2(REG_PARAM), padding="same", input_shape=(M*T + L, N, N), data_format="channels_first")(self.input)
        norm = BatchNormalization(axis=1)(conv)  # Batch normalisation
        ...
        policy_conv = Conv2D(2, kernel_size=1, strides=1, activation='relu', kernel_regularizer=l2(REG_PARAM), padding="same", data_format="channels_first")(tower)
        policy_norm = BatchNormalization(axis=1)(policy_conv)  # Batch normalisation
        policy_rnln = ReLU()(policy_norm)  # A rectifier non-linearity
        # Make sure the output size is correct
        policy_flat = Flatten(data_format="channels_first")(policy_rnln)
        ...
        value_conv = Conv2D(1, kernel_size=1, strides=1, activation='relu', kernel_regularizer=l2(REG_PARAM), padding="same", data_format="channels_first")(tower)
        value_norm = BatchNormalization(axis=1)(value_conv)  # Batch normalisation
        value_rnln1 = ReLU()(value_norm)  # A rectifier non-linearity
        # Make sure the output size is correct
        value_flat = Flatten(data_format="channels_first")(value_rnln1)
    ...
    def evaluate(self, states):
        input_planes = np.empty((M*T + L, N, N))
...
def get_piece_planes(board):
    ...
    result = np.zeros((12, N, N))
    for rank in range(N):
        for file in range(N):
            piece = state[rank * N + file]
            if piece.isalpha():
                result[fen_order.find(piece)][rank][file] = 1
    return result
